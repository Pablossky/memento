...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = MementoAdapter(
            ...
            onClick = {
                findNavController().navigate(
                    HomeFragmentDirections.actionHomeFragmentToMementoEditorFragment(it.memento.id.toInt())
                )
            },
            onLongClick = { item ->
        ...
        binding.fabAdd.setOnClickListener {
            findNavController().navigate(
                HomeFragmentDirections.actionHomeFragmentToMementoEditorFragment(0)
            )
        }
            ...
            override fun onCreateMenu(menu: Menu, menuInflater: MenuInflater) {
                ...
                sv.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                    ...
                    override fun onQueryTextChange(q: String?): Boolean {
                        viewModel.setSearchQuery(q.orEmpty()); return true
                    }
                })
            }
        ...
        viewModel.allCategories.observe(viewLifecycleOwner) { categories ->
            ...
            categories.forEach { category ->
                binding.chipGroupCategories.addView(Chip(requireContext()).apply {
                    ...
                    setOnClickListener {
                        val newCat = if (viewModel.selectedCategory.value == category.id) null else category.id
                        viewModel.setSelectedCategory(newCat)
                    }
                })
            }
        }
